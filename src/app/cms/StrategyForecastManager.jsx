...
function findDefaultStrategyForecastCategory(items) {
  return items.find((item) => {
    const en = String(item.name?.en || "").trim().toLowerCase();
    const ar = String(item.name?.ar || "").trim();
    return en === "strategy forecast" || ar === "التوقعات الاستراتيجية";
  });
}

export default function StrategyForecastManager() {
  ...
  const activeCategory = useMemo(
    () => categories.find((item) => item.metadata?.strategyForecast?.enabled),
    [categories]
  );
  ...
  const load = async () => {
    ...
    const active = flatCategories.find((item) => item.metadata?.strategyForecast?.enabled);
    const defaultCategory = findDefaultStrategyForecastCategory(flatCategories);
    const resolvedCategory = active || defaultCategory || null;
    setSelectedCategoryId(resolvedCategory?._id || "");
    setDefaultProgress(String(resolvedCategory?.metadata?.strategyForecast?.defaultProgress ?? 0));
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const safeProgress = Math.max(0, Math.min(100, Number(defaultProgress) || 0));
      ...
      await load();
      setMessage({ type: "success", text: "Strategy Forecast mapping saved." });
    } catch (error) {
      console.error(error);
      setMessage({ type: "error", text: "Failed to save Strategy Forecast mapping." });
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <Stack spacing={2.5}>
          ...
          <Typography color="text.secondary">
            Choose the saved category that should open the Strategy Forecast experience on the
            controller and mirror it on the big screen.
          </Typography>

          {!activeCategory && selectedCategoryId && (
            <Alert severity="info">
              A category named Strategy Forecast was detected and preselected for mapping.
            </Alert>
          )}
          ...
          <TextField
            label="Default slider progress"
            type="number"
            inputProps={{ min: 0, max: 100 }}
            value={defaultProgress}
            onChange={(event) => setDefaultProgress(event.target.value)}
            helperText="Initial slider position when the experience opens."
            fullWidth
          />
